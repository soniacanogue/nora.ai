...
const StatCard = ({
  ...
  return (
    <div className={`${containerClasses} ${className}`}>
      ...
      <div
        className={`relative ${isCard ? "p-5" : ""} flex items-start justify-between`}
      >
        <div>
        ...
        <div
          className={`p-3 rounded-lg ${isCard ? "bg-white/5 border border-white/5" : "bg-transparent"} text-dt-accent group-hover:text-white group-hover:bg-dt-accent transition-colors duration-300`}
        >
          {icon}
        </div>
      </div>
