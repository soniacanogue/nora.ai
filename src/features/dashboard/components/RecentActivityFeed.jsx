...
const RecentActivityFeed = ({ activities }) => {
  return (
    <div className="bg-dt-gray-800/50 rounded-lg p-4 space-y-4">
        ...
        activities.map((activity) => {
          ...
          const content = (
            <>
              ...
              <p className="text-dt-subtle text-dt-xs">
          ...
          if (activity.ticketId) {
            return (
              <Link
                ...
                className="block text-dt-sm hover:bg-dt-gray-700/50 p-2 rounded transition-colors"
              >
                {content}
              </Link>
            );
          ...
          return (
            <div key={activity.eventId} className="text-dt-sm p-2">
              {content}
            </div>
          );
        })
      ) : (
        <p className="text-dt-subtle text-dt-sm">No hay actividad reciente.</p>
      )}
    </div>
  );
};
