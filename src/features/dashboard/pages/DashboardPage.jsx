...
const DashboardPage = () => {
  ...
  if (!dashboardData) {
    return <EmptyState message="No se encontraron datos para el dashboard del agente." />;
  }

  const { myMetrics, myQueues, myTicketsByStatus, recentActivity } = dashboardData;
  
  // Select metrics based on time range
  const currentMetrics = timeRange === "today" ? myMetrics.today : myMetrics.thisWeek;
